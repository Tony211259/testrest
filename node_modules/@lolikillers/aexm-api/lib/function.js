...
function delta(query){
    ...
    axios.get(query)
        .then(({ data }) => {
            const $ = cheerio.load(data)
                const a1 = $('div.detail_wrap > div.detail_text').find('p').text()
                const img = $('div.media_artikel').find('img').attr("src")
                const title = $('div.media_artikel').find('img').attr("title")
                const time = $('div.content_detail').find('div.date').text()
                const result = {
                		title: title,
                		released: time,
                		image: img,
