...
function trs(text, too){
    return new Promise(async(resolve, reject) => {
        const hsl = translate(text, {to: too})
...
function youtube(url){
    return new Promise(async(resolve, reject) => {
        ytdl.getInfo(url).then(async(getUrl) => {
            ...
            let ads = await ytv(url, '720p').dl_link
        ...
        }).catch(reject); console.log(ads)
    });
};

function ytmp4(url){
return new Promise(async(resolve, reject) => {
            let media = await ytv(url, '720p')
            const hsl = media.dl_link
       resolve(hsl)
       console.log(hsl)
